...
# для нахождения максимального числа ягод, которое может собрать за 
# один заход собирающий модуль, находясь перед некоторым кустом заданной 
...
for i in range(len(list_1)):
    list_sum.append(list_1[i - 2] + list_1[i-1] + list_1[i])
tmp = 0
for i in range(len(list_sum)):
    if list_sum[i-1]>list_sum[i]:
        tmp = list_sum[i-1]
        list_sum[i-1] = list_sum[i]
        list_sum[i] = tmp    
print(list_sum[i])
